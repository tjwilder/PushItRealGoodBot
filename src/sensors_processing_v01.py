...
# 75.81262327416174  # standard ME439 robot: gear_ratio =
# 75.81262327416174 = (34.*34.*35.*38.)/(13.*12.*13.*10.)
gear_ratio = rospy.get_param('/gear_ratio')
encoder_counts_to_radians = 1. / \
    counts_per_encoder_revolution / gear_ratio * 2.0 * np.pi
...
pub = rospy.Publisher('/sensors_data_processed',
...
def listener():
    ...
    rospy.Subscriber('/sensors_data_raw', ME439SensorsRaw, sensors_process)
...
def sensors_process(msg_in):
    ...
    global t_previous, e0_previous, e1_previous, e0_offset, e1_offset
    global encoder_counts_to_radians, initializing, pub
    ...
    try:
        if initializing:
            ...
            msg_out.e0radpersec = (e0_current - e0_previous) / (
                dt) * encoder_counts_to_radians
            msg_out.e1radpersec = (e1_current - e1_previous) / (
                dt) * encoder_counts_to_radians
