...
def animate_robot(robot):
    ...
    # Note for some reason this has to come before the "xlim" and "ylim" or an
    # explicit axis limits command "plt.axis([xmin,xmax,ymin,ymax])"
    plt.axis('equal')
    plt.xlim(-1, 1)
    plt.ylim(-1, 1)
    plt.xlabel('x')
    ...
    line_ani = animation.FuncAnimation(fig1,
                                       update_drawing,
                                       frames=1,
                                       fargs=(robot, robotoutline, robotpath),
                                       interval=(dt * 1000),
                                       repeat=True,
                                       blit=False)
    plt.show()
...
def update_drawing(
    num, robot, robotoutline, robotpath
):  # "num" is the current frame number - doesn't do anything in this case
    ...
    # Find limits of the plot and expand them to make sure the robot is on
    # screen
    axlims = plt.axis()
    xmin = axlims[0]
    xmax = axlims[1]
