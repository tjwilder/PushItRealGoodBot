...
class quadrature_encoder:
    def __init__(self, serial_string_identifier, counts_per_encoder_revolution,
                 gear_ratio, wheel_radius, forward_encoder_rotation_sign):
        # serial_string_identifier is e.g. "E0" or "E1" does nothing, just an
        # identifier that we hope the user will keep as a unique link to the
        # sensor data streaming in.
        self.serial_string_identifier = serial_string_identifier
        ...
        self.counts_to_radians_multiplier = (
            float(1) / self.counts_per_encoder_revolution / self.gear_ratio * 2
            * np.pi * self.forward_encoder_rotation_sign)

        self.t = rospy.get_rostime()
        self.t_previous = self.t
        # (self.t - self.t_previous).to_sec()
        self.dt = 0.01
        self.counts = 0
        ...
        # this is a flag that will make "update" run twice (to set both
        # position and per_second) the first time it is called.
    ...
    def update(self, counts_measured, dt):
        ...
        if self.initializing:
            ...
            # Prevent the rest of the function from running on the
            # Initialization call.
            return
        ...
        self.counts_per_second = float(self.counts
                                       - self.counts_previous) / self.dt
        # self.radians_per_second_previous = self.radians_per_second
        self.radians_per_second = self.counts_per_second * \
            self.counts_to_radians_multiplier
        # self.meters_per_second_previous = self.meters_per_second
        self.meters_per_second = self.radians_per_second * self.wheel_radius


# a general purpose FPID Velocity controller, to be instantiated in
# whatever way the user desires.
class FPID_controller():
    def __init__(self,
                 ...
                 Ki=0.,
                 Kd=0.,
                 error_integral_limit=np.inf,
                 integral_resetting=True,
                 motor_command_max_change_rate=100000.,
                 motor_command_max=480.,
        ...
        self.motor_command_max_change_rate = motor_command_max_change_rate
...
#            self.error_integral = 0.
    ...
    def compute_error_derivative(self):
        self.error_derivative = float(self.error
                                      - self.error_previous) / self.dt

    def compute_command(self):
        # With FeedForward Gain
        self.motor_command = self.forward_motor_command_sign * (
            self.Kf * self.target + self.Kp * self.error
            + self.Ki * self.error_integral + self.Kd * self.error_derivative)

    def limit_command_rate_of_change(self):
        motor_command_rate_requested = (
            self.motor_command - self.motor_command_previous) / self.dt
        motor_command_rate = np.clip(motor_command_rate_requested,
                                     -self.motor_command_max_change_rate,
                                     self.motor_command_max_change_rate)
        self.motor_command = (self.motor_command_previous
                              + self.dt * motor_command_rate)

    def limit_command(self):
        self.motor_command = np.clip(self.motor_command,
                                     -self.motor_command_max,
                                     self.motor_command_max)
